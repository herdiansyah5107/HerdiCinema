...
public class SciFiFragment extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        String[] cinemaName = new String[Film.scifi.length];
        for (int i = 0; i < cinemaName.length; i++) {
            cinemaName[i] = Film.scifi[i].getName();
        }
        int[] cinemaImage = new int[Film.scifi.length];
        for (int i = 0; i < cinemaImage.length; i++) {
            cinemaImage[i] = Film.scifi[i].getImageResourceId();
        }
